---
curso: ML
titulo: swiss_roll_viz
slides: 0
fuente: swiss_roll_viz.py
---

# Swiss Roll y Moons: los límites de PCA frente a la estructura no lineal

Este script genera dos datasets sintéticos clásicos y los usa para mostrar, de forma visual, qué puede y qué no puede capturar el PCA. Primero construye un **Swiss Roll** con `make_swiss_roll` (1500 puntos, una variedad 2D enrollada dentro de R³) y un conjunto **Moons** que empieza en 2D y se infla a 30 dimensiones agregando 28 columnas de ruido gaussiano con `np.hstack`. Estandariza con `StandardScaler` y aplica `PCA` para proyectar de 3D a 2D. Luego arma una figura de seis paneles: dos vistas 3D del rollo (coloreadas por clase y por el parámetro continuo `t` de la espiral), la proyección PCA 2D, dos vistas ortogonales (superior X-Z y lateral X-Y) y el gráfico de varianza explicada acumulada. Una segunda figura repite el ejercicio sobre Moons: original 2D, proyección PCA desde R³⁰ y curva de varianza acumulada.

El concepto central es **reducción de dimensionalidad lineal** y su punto ciego: PCA busca las direcciones de máxima varianza asumiendo linealidad, por eso "mezcla las clases" del Swiss Roll y pierde la estructura enrollada, tal como advierte la anotación del código. Enseña a leer la varianza explicada como criterio de cuántas componentes retener (líneas de 80/90/95%).

En un proyecto real sirve para decidir cuándo PCA basta y cuándo hace falta un método no lineal (t-SNE, UMAP, Isomap). Conecta con preprocesamiento, visualización exploratoria y clasificación.
