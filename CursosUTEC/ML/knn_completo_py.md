...
titulo: knn_completo
slides: 0
fuente: knn_completo.py
---

# KNN desde cero: el sesgo, la varianza y el porqué de K

Este archivo implementa **K-Nearest Neighbors** sin apoyarse en el clasificador de sklearn. El núcleo es una clase `KNN` que expone la lógica más honesta del algoritmo: `fit` no entrena nada, solo almacena `X_train` y `y_train`; toda la computación ocurre en predicción. Para clasificar un punto, calcula su **distancia euclidiana** a cada muestra de entrenamiento, toma con `argsort` los `k` índices más cercanos y decide por **votación mayoritaria** con `Counter`. Es el ejemplo canónico de un algoritmo *lazy* y no paramétrico: no hay pesos ni función que ajustar, la frontera de decisión emerge directamente de los datos.

El concepto que ejercita es la **tensión sesgo-varianza gobernada por el hiperparámetro K**. Los cinco casos son un experimento controlado: con datos bien separados `K=√n` es óptimo; con `K` muy grande la frontera se sobre-suaviza (**underfitting**); con `K=1` el modelo memoriza el ruido (**overfitting**), reforzado inyectando 10% de etiquetas erróneas. Las curvas de *accuracy* train vs. test hacen visible la brecha. Iris y Wine llevan el ejercicio a datos reales, dejando que el alumno busque el `K` óptimo y verifique si coincide con la heurística `√n`.

En un proyecto real esto sustenta la elección de hiperparámetros por validación, la importancia de **normalizar** (min-max solo con estadísticas de train, evitando fuga de datos) y la lectura de fronteras de decisión. Conecta con evaluación de modelos, regularización y validación cruzada.
