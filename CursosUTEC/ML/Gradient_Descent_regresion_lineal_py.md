---
curso: ML
titulo: Gradient_Descent_regresion_lineal
slides: 0
fuente: Gradient_Descent_regresion_lineal.py
---

# Regresión lineal desde cero: Gradient Descent y SGD

Este código implementa regresión lineal simple `f(x) = wx + b` **sin librerías externas** (nada de sklearn ni pytorch), con fines puramente pedagógicos, sobre un dataset de niveles de azúcar contra glucosa en sangre. El objetivo es ajustar los parámetros `w` (pendiente) y `b` (sesgo) para minimizar el error cuadrático medio (MSE).

Lo que hace paso a paso: define funciones base para predecir, medir error (MSE y MAE) y calcular el gradiente del MSE respecto a `w` y `b`. Luego ofrece tres formas de resolver el mismo problema y las contrasta. **Batch Gradient Descent** usa todos los datos en cada iteración para calcular el gradiente y actualiza los parámetros con un `learning rate`. **Stochastic Gradient Descent** actualiza usando una sola muestra aleatoria por paso, con shuffle por época y semilla fija para reproducibilidad. Y la **solución analítica** por mínimos cuadrados, que sirve de referencia exacta para verificar que GD y SGD convergen a lo mismo.

El concepto de ML que ejercita es el núcleo del aprendizaje por optimización: función de costo, gradiente, tasa de aprendizaje y convergencia. El `main` muestra explícitamente cómo `lr=0.1` diverge y `lr=0.005` es estable, más iteraciones manuales detalladas que exponen el cálculo del gradiente término a término.

En un proyecto real esto es la base de casi todo el entrenamiento supervisado moderno: conecta con funciones de pérdida, descenso de gradiente en redes neuronales, mini-batch, y el trade-off entre precisión, ruido y velocidad de convergencia.
